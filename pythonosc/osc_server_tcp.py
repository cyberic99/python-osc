...
class _TCPHandler(socketserver.BaseRequestHandler):
  ...
  def handle(self):
    ...
    lengthbuf = recvall(self.request, 4)
    length, = struct.unpack('!I', lengthbuf)
    data = recvall(self.request, length)

    #self.request = (self.request, data)
    #print("h1 ", self.request)
    #print("h2 ", type(self.request))
    _call_handlers_for_packet(data, self.server.dispatcher)
...
def _is_valid_request(request):
  ...
  data = request[0]
  res = (osc_bundle.OscBundle.dgram_is_bundle(data) or osc_message.OscMessage.dgram_is_message(data))
  return res
